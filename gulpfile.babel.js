...
gulp.task("build", gulp.series(clean, pages, sass, images, inline, fonts));
...
gulp.task("default", gulp.series("build", server, watch, fonts));
...
function maximizer() {
  var pipe = lazypipe().pipe($.prettyHtml);
  return pipe();
}
// end
...
function minifyEmailCopy() {
  return gulp.src(["dist/*.html"]).pipe(gulp.dest("./dist/minEmail/"));
}
